...
// NOTE: Uses ColorIndex (int) + level-level ColorScheme instead of VineColor (string).
type Vine struct {
	ID            string  `json:"id"`
...
// Module represents a module with parable content and level list.
// NOTE: Uses Levels ([]int) + ChallengeLevel (int) instead of LevelRange ([2]int).
type Module struct {
	ID             int      `json:"id"`
	...
	Parable        *Parable `json:"parable,omitempty"`
	UnlockMessage  string   `json:"unlock_message,omitempty"`
}

// ModuleRegistry represents the complete modules.json structure.
