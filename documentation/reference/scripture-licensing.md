...
- Every scripture citation displayed in the user interface (e.g., daily scripture banners, level complete rewards, journal pages) must include the book name, chapter, verse, and translation abbreviation:
  - _Example:_ _"A joyful heart is good medicine, but a crushed spirit dries up the bones."_ **Proverbs 17:22 (ESV)**
...
3. **Network/Offline Fallback Router:**
   - The mobile app routes scripture fetches through a unified service provider:
     ...
     - If the device is offline, the app dynamically displays a banner: _"This translation requires an internet connection for copyright compliance. Switch to World English Bible (WEB) to read offline."_ Tapping the banner toggles the active version to **WEB** (or KJV) and loads the offline verse from local storage.
